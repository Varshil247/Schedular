...
#!class of a page which runs the student databse management system
class Page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.logo = tk.Label(self.topFrame, text = 'Schedular > Admin Dashboard > Manage Students', font=LargeFont)
        ...
        #!creating the student database viewing treeview widget
        self.createStudentDBViewer()
        self.createDataFrame()
        self.createCommandsFrame()

#!########################################################################
#!function that creates a database style treeview widget to view the student records
    def createStudentDBViewer(self):
        ...
        #!student database widget frame
        self.dbWidget = tk.Frame(self.mainframe, background = '#2c2f33')
        self.dbWidget.grid(row=1, column=0, padx = 20, pady = 10, sticky='ew')

        #!scrollbar on right side of the dbwidget
        ...
        #!configuring the scrollbar to view/scroll in the y axies of the dbviewer
        self.tree_scroll.config(command=self.dbViewer.yview)

        #!setting the column titles to the fields in the student database
        self.dbViewer['columns'] = ['Student ID','Username','Password','Firstname','Surname']
...
#!########################################################################
    def createDataFrame(self):
        ...
        self.data_frame.grid(row=2, column=0, padx = 20, pady = 10, sticky='ew')

        #!id(integer) entry for the student record
...
#!########################################################################
    def createCommandsFrame(self):
        ...
        self.commands_frame.grid(row=3, column=0, padx = 20, pady = 10, sticky='ew')
        ...
        self.warningLbl.grid(row=5, column=0, columnspan=5, padx = 20, pady = 10, sticky='w')

#!########################################################################
    #!redirects user back to admin dashboard page,
    #!clears all activities carried out by Admin, to keep privacy of data
    #!wipes out any data in the dbviewer and form inputs, sets warning message back to default
    def back(self):
        self.controller.show_frame(AdminDashboard.Page)
        self.clearEntries()
        self.clearCells()
        self.warningLbl.config(text='No Changes.', fg='#7289da')

#!########################################################################
#!All backend operations for database management
    #!shows records of the student database on the dbviewer
    def showData(self):
        self.clearEntries()
        self.createStudentDBViewer()
        cur.execute('SELECT * FROM Student')
        for row in (cur.fetchall()):
            self.dbViewer.insert(parent='', index='end', values=(row[0],row[1],row[2],row[3],row[4]))
        self.warningLbl.config(text='Student Records Displayed.', fg='#7289da')
    ...
    def addData(self):
        ...
        if userExists == False:
            ...
            if self.usernameEntry.get() != '' and self.passwordEntry.get() != '' and self.firstnameEntry.get() != '' and self.surnameEntry.get() != '':
                newData = (self.usernameEntry.get(), encryptedPassword, self.firstnameEntry.get(), self.surnameEntry.get())
                cur.execute('INSERT INTO Student(Username,Password,Firstname,Surname) VALUES(?,?,?,?)', newData)
                self.showData()
    ...
    #!deletes the record selected from the dbviewer and the database itself
    def deleteData(self):
        selected = self.dbViewer.focus()
        values = self.dbViewer.item(selected, 'values')

        if selected != '':
            cur.execute('DELETE FROM Student WHERE StudentID=?', values[0])
            cur.commit()
            self.showData()
